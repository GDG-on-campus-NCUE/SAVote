...
2.  **使用者儀表板 (User Dashboard)**
    *   **功能:**
        *   顯示使用者基本狀態 (已驗證/未驗證)。
        *   **選舉列表:** 分為「進行中」、「即將開始」、「已結束」。
        *   **投票狀態:** 標示每場選舉是否已投票。

3.  **選舉詳情頁 (Election Detail)**
    *   **功能:**
        *   選舉公告、規則說明、起訖時間。
        *   **候選人列表:** 候選人照片、政見、經歷介紹。
...
4.  **投票間 (Voting Booth)**
    *   **核心功能:**
        ...
        *   **選票提交:** 將 Proof 與加密選票送至後端。
    *   **防護:** 防止頁面重整導致狀態遺失，提交後自動跳轉至成功頁。

5.  **投票成功/憑證頁 (Success & Receipt)**
    *   **功能:**
        *   顯示投票成功訊息。
        *   **數位存根:** 提供 Nullifier Hash 或交易憑證供使用者下載/截圖，作為日後驗票依據。

6.  **驗證中心 (Verification Center)**
    *   **功能:**
        *   **個人驗票:** 輸入 Nullifier Hash 查詢選票是否被計入。
        *   **全域驗證:** 下載完整計票日誌與 ZK Proofs，供第三方工具驗證。
...
2.  **選舉人名冊 (Voter Roll)**
    *   匯入合資格名單 (透過 CSV 或 API 同步)。
    *   設定 Merkle Tree Root (若採用 Merkle Tree 驗證資格)。
